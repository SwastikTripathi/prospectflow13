...
interface AddJobOpeningDialogProps {
  ...
  onAddNewCompany: (companyName: string) => Promise<Company | null>; // This prop will be called by parent onSubmit
...
export function AddJobOpeningDialog({
  ...
  contactsCount,
  jobOpeningsCount,
  onAddNewCompany, // Prop for parent to handle DB creation
  onAddNewContact,
  ...
  const stageNewCompanyInFormAfterLimitCheck = (name: string): boolean => {
    ...
    form.setValue("company_id", "", { shouldValidate: true }); // Clear ID to indicate it's new
  ...
  const handlePotentialNewContactSelection = (contactName: string, index: number): boolean => {
    ...
    form.setValue(`contacts.${index}.contact_id`, undefined, { shouldValidate: true }); // Mark as new
